CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    TYPES tt_travel_update TYPE TABLE FOR UPDATE zi_travel_shu.

    METHODS calculatetravelkey FOR DETERMINATION travel~calculatetravelkey
      IMPORTING keys FOR travel.

    "METHODS validate_customer FOR VALIDATE ON SAVE "FOR VALIDATION Travel~validateCustomer
    "    IMPORTING keys FOR Travel~validateCustomer. "Travel.
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD validate_customer.
    ...
    READ ENTITY IN LOCAL MODE zi_travel_shu\\travel FROM VALUE #(
        FOR <root_key> IN keys ( %key     = <root_key>
                                 %control = VALUE #( customer_id = if_abap_behv=>mk-on ) ) )
        RESULT DATA(lt_travel).
    IF lt_travel IS INITIAL OR lt_travel[ 1 ]-customer_id IS INITIAL.
      APPEND VALUE #(  mykey = lt_travel[ 1 ]-mykey ) TO failed.
      APPEND VALUE #(  mykey = lt_travel[ 1 ]-mykey
                       %msg  = new_message( id       = /dmo/cx_flight_legacy=>customer_unkown-msgid
                                            number   = /dmo/cx_flight_legacy=>customer_unkown-msgno
                                            v1       = lt_travel[ 1 ]-customer_id
  ...
  METHOD set_status_completed.
    ...
    READ ENTITIES OF zi_travel_shu IN LOCAL MODE
         ENTITY travel
  ...
  METHOD get_features.
    ...
    READ ENTITY IN LOCAL MODE zi_travel_shu FROM VALUE #( FOR keyval IN keys
                                                      (  %key = keyval-%key
